    ...
    def __init__(
        ...
        Args:
            ...
            s3_kms_key (str, optional): KMS key for S3. Per the API documentation
                ...
                if not provided, SageMaker uses the default KMS key for Amazon S3 of the account
                linked to your IAM role. Also according to the same documentation, the accepted
                formats are (1) KMS Key ID, (2) ARN of a KMS key, (3) KMS Key alias, and (4) ARN of
                a KMS key alias. Defaults to "abcde".
            sgs (List[str], optional): List of security groups. Defaults to
                ["sg-abcde"].
            volume_kms_key (str, optional): KMS key for EBS volume on job instances. You cannot use
                an AWS Managed Key such as ``alias/aws/ebs`` because its key policy cannot be
                edited, so cross-account permissions cannot be granted for these key policies.
                Please ensure that the customer-managed key allows cross-account use in its policy
                ...
                Also note that when using instances with local NVMe (e.g., instance type with ``d``,
                you must set this to `False` otherwise the ``Create*Job`` API will fail (refer to
                the before-mentioned documentation URL). Defaults to "abcde".
            encrypt_inter_container_traffic (bool): Whether to traffic between job containers is
                encrypted. Applicable for training and processing. For implementer of this class:
                model monitor requires inter-container traffic encryption disabled (see
                `sagemaker.model_monitor.model_monitoring.ModelMonitor._validate_network_config()`).
                Hence, implementers must ignore this flag when setting the `NetworkConfig` for model
